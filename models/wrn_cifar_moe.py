...
class WideResNet(MoEBase):
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0, n_expert=5, ratio=1.0):
        ...
        self.conv1 = nn.Conv2d(
            3, int(nChannels[0] * ratio), kernel_size=3, stride=1, padding=1, bias=False
        )
        # 1st block
        self.block1 = NetworkBlock(
            n, int(nChannels[0] * ratio), int(nChannels[1] * ratio), block, conv_layer, 1, dropRate, n_expert=n_expert
        )
        # 2nd block
        self.block2 = NetworkBlock(
            n, int(nChannels[1] * ratio), int(nChannels[2] * ratio), block, conv_layer, 2, dropRate, n_expert=n_expert
        )
        # 3rd block
        self.block3 = NetworkBlock(
            n, int(nChannels[2] * ratio), int(nChannels[3] * ratio), block, conv_layer, 2, dropRate, n_expert=n_expert
        )
        # global average pooling and classifier
        self.bn1 = nn.BatchNorm2d(int(nChannels[3] * ratio))
        self.relu = nn.ReLU(inplace=True)
        self.fc = linear_layer(int(nChannels[3] * ratio), num_classes)
        self.nChannels = int(nChannels[3] * ratio)
